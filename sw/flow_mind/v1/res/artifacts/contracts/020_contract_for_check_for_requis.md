```yaml
title: 'Contract: Requirements Check for Requis'
tags:
- contract
- requirements
- completeness
- consistency
persona: gatekeeper
status: active
version: V00.01.00
updated: 2026-06-08
summary: 'Plan for checking completeness and consistency of requirements'
```

# Contract: Requirements Check for Requis

## 1. Completeness Check

### 1.1 Coverage Analysis
- Verify all requirements in `*/requi/` have corresponding implementation
- Cross-reference requirement IDs with test cases
- Ensure no orphaned requirements exist

### 1.2 Dependency Mapping
- Map each requirement to affected components
- Identify missing prerequisite requirements
- Validate requirement hierarchy (parent-child relationships)

### 1.3 Traceability Matrix
- Create matrix: Requirement → Specification → Implementation → Test
- Flag requirements without traceability links

## 2. Consistency Check

### 2.1 Cross-Document Validation
- Check for conflicting requirements across files
- Validate terminology consistency using glossary
- Ensure no duplicate requirements exist

### 2.2 Structural Consistency
- Verify all requirement documents follow standard format
- Validate YAML frontmatter compliance
- Check section structure alignment

### 2.3 Version Consistency
- Ensure requirement versions align with implementation
- Validate change history entries

## 3. Execution Workflow

```yaml
workflow:
  id: requirements_check
  name: "Requirements Check"
  type: flow
  extends: 010_flow_base
  states:
    - name: init
      transitions:
        - to: check_completeness
    - name: check_completeness
      transitions:
        - to: check_consistency
    - name: check_consistency
      transitions:
        - to: generate_report
    - name: generate_report
      transitions:
        - to: complete
    - name: complete
```

## 4. Acceptance Criteria

- [ ] All requirements traced to implementation
- [ ] No conflicting requirements found
- [ ] All documents follow standard format
- [ ] Glossary terms used consistently

## 5. Tools & Methods

| Tool | Purpose |
|------|---------|
| `grep` | Find requirement references |
| `diff` | Compare requirement versions |
| YAML validator | Check document structure |
| Custom script | Generate traceability matrix |

## 6. Cadence

- Run on every release
- Run after major requirement changes
- Quarterly baseline check